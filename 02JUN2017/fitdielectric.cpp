...
vector<vector<double>> FitDielectric::dielectricData_preprocess(const StructureClass& sysVar)
{
    ...
    if (form=="NPIC")
    {
        ...
        {
            spcstr.clear();tempstr.clear();extstr.clear();
        ...
        for (int species=0;species<(int)species_2d.size();++species)
        {
            vector<double> Temps;
            ...
            for (int T=0;T<temp_2d.at(species).size();++T)
            {
                ...
                output.append("species_"+species_2d.at(species).at(0)+"_");
                ...
                overall.append("/DSdata_species_"+species_2d.at(species).at(0)+"_");
                overall.append(".dat");
                ...
                input.append(return_AnalysisFolderPath(sysVar));
                input.append("/statistics/dielectric/");
                input.append(species_2d.at(species).at(0)+"-");
                ...
                if (readFile.is_open())
                {
                    ...
                    while (getline(readFile,lineContent))
                    {
                        ...
                        {
                            tmpvvd.push_back({2*pi()*sfreq,sfreq,epspp,epsp});
                                             ...
                                             double Temp_d)
{
    ...
    if (form=="NPIC")
    {
        ...
        {
            cout << "\n";
            cout << "species#   ("<<get_spc_id().at(0)+1<<"/"<<get_spc_id().at(1)<<")\n";
            cout << "Temp#      ("<<get_T_id().at(0)+1  <<"/"<<get_T_id().at(1)  <<")\n";
            cout << "T(Kelvin)  ("<< spc_Temps.at(spc_id).at(T_id)     << ")\n";
            cout << "T(Celcius) ("<< spc_Temps.at(spc_id).at(T_id)-273 << ")\n";
            //for (int i=0;i<dielectric_freq.size();++i) {
            //    for (int ii=0;ii<dielectric_freq.at(i).size();++ii) {
            //        cout << dielectric_freq.at(i).at(ii) << " ";
            //    } cout << "\n";
            //}
        }
    }
...
void FitDielectric::peak_sampling_bySlope(const vector<vector<double>>& data)
{
    ...
    //build_cubicspline_loglog_interpolant(data,0,2);
...
void FitDielectric::dc_curve_sampling(const vector<vector<double>>& data)
{
    ...
    //cout << "peak_indices_original() "<<peak_indices_original.size()<<"\n";
    ...
    //cout << "firstLossPeakii " << firstLossPeakii << "\n";
    ...
    if (firstLossPeakii>n_check) {
        ...
        } //cout << "base_indexii " << base_indexii << "\n";
        //**********************************************************************
...
{
    for (indexii=0; indexii<(int)dielectric_freq_overall.size(); ++indexii) {
        afreqi=dielectric_freq_overall[indexii][0];
        loss  =dielectric_freq_overall[indexii][2];
        if (logafreq==log10(afreqi)) break;
    } return loss;
}
                                        ...
                                        double Temp_d)
{
     ...
     * 'dielectric_freq_overall' that stores complete dielectric reponse of one T;
     * dielectric_freq_overall has the structure: {afreq,sfreq,loss,storage}
     * all in its original value (ie. no log scale) */
    ...
    if( n_dcpoints>1 || n_peaks>0 )
    {
        ...
        counter=0;
        while ( r2_global_pre<r2_threshold && counter<5 ) { // NOTE: which threshold
            ...
            //cout << "r2_global_pre " << r2_global_pre << "\n";
            //cout << "r2_global_now " << r2_global_now << "\n";
            
            if(r2_global_now<=r2_global_pre) {
                /* if further global fit doesn't increase corresponding r2,
