...
void FitDielectric::peak_sampling_bySlope(const vector<vector<double>>& data)
{
    ...
    if( n_reduction>0 && peak_indices.size()>0 ) {
        ...
        /* sort to find the highest peak in reduced loss */
        ...
        /* the loss peak needs to be above threshold value
         NOTE: threshold value is based on raw loss */
    ...
    if (n_peaks>0) {
        for (whichpeak=0; whichpeak<n_peaks; ++whichpeak) {
            for (int i=0; i<(int)dielectric_freq_sortincreasing.at(whichpeak).size(); ++i) {
                ...
                //logloss =dielectric_freq_sortincreasing.at(whichpeak)[i][1]; // log(rduLoss)
...
{
    /* sample data to fit in the proximity of found peaks */
    if (n_reduction==0) { /* peak detection of raw loss  */
        peak_sampling_bySlope(dielectric_freq_overall);
    } else { /* peak detection of reduced loss */
                                        ...
                                        double Temp_d)
{
    ...
    if( n_dcpoints>1 || n_peaks>0 )
    {
        ...
        while (r2_global_pre<r2_threshold) { // NOTE: which threshold
            ...
            if (dielectric_freq_reduced.size()>3) {
                ...
                if(r2_global_now<=r2_global_pre) {
                    /* if further global fit doesn't increase fitting effectiveness,
                     break the while loop and use the highest achieved global fit */
                    //n_reduction -= 1;
                    //n_peaks=n_peaks_pre;
                    break;
                } else {
                                         ...
                                         void *ptr)
{
    ...
    try {
        ...
        cout
        << "in FitDielectric::static_combined_func: c.length() is invalid."
        << "\n";
        system("read -t5");exit(EXIT_FAILURE);
    }
}
