...
cd /Users/SJH/Dropbox/UA_Research/Codes/cppWork/cppWork/simulations/recover/2015/recovertest/analysis

for i in ${trial[@]}; do
  for ii in ${namestring}; do
    for iii in ${Temp[$i]}; do
      qsub_File=./AMDAT_submission_files/AMDAT_recovertest_00${i}_${ii}_T${iii}.qsub
      test -e ${qsub_File}
      if [ "$?" -eq "0" ]; then
        qsub ${qsub_File}
      else
