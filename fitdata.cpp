...
void FitData::set_fitParams(const std::string& model)
{
    ...
    else if (model=="Generic_MYEGA") // [tau0,a,Tg,n,taug]
    {
        if (systemUnit=="real")
        {
        ...
        {
            set_coeffs({        0.1,  5.0, 3e+2, 1.0, 1e+14});
            set_coeffs_scale({  1.0,  1.0, 1e+2, 1.0, 1e+14});
            set_coeffs_bndl({   0.0,  0.0,  0.0, 0.0, 1e+14});
            set_coeffs_bndu({   inf,  inf,  inf, inf, 1e+14});
        }
        else if (systemUnit=="lj")
        {
            set_coeffs({        0.1,  5.0,  0.3, 1.0, 1e+14});
            set_coeffs_scale({  1.0,  1.0,  0.1, 1.0, 1e+14});
            set_coeffs_bndl({   0.0,  0.0,  0.0, 0.0, 1e+14});
            set_coeffs_bndu({   inf,  inf,  inf, inf, 1e+14});
        }
    }
    else if (model=="Generic_COOP") // [tau0,a,Tg,n,taug]
    {
        if (systemUnit=="real")
        {
            set_coeffs({       1e+2,  5.0, 3e+2, 1.0, 1e+17});
...
void FitData::Generic_MYEGA_func(const real_1d_array &c,
                                 const real_1d_array &x,
                                 double &func,
                                 void *ptr)
{
                                 ...
                                 void *ptr)
{
    ...
    double B=(-1+(Tg/T))*pow(T/Tg,n);
    grad[1]= (A*B*exp(a*B))*log10(exp(1));//a
    cout << "in FitData::Generic_MYEGA_grad(): please use F-mode\n";exit(1);
    grad[2]= (1)*log10(exp(1));//Tg
    grad[3]= (1)*log10(exp(1));//n
    grad[4]= (1)*log10(exp(1));//taug
}
...
void FitData::Generic_COOP_func(const real_1d_array &c,
                                const real_1d_array &x,
                                double &func,
                                void *ptr)
{
                                       ...
                                       const string& model)
{
    ...
    {
        /*===========================================
         c[0]=tau0, c[1]=a, c[2]=Tg, c[3]=n, c[4]=taug
         ===========================================*/
        glass_data.push_back(c[2]);//Tg
        
        //a= (m/(log10(taug/tau0)))-1
        double m=(c[1]+1)*log10(c[4]/c[0]);//m
        glass_data.push_back(m);
    }
    else if (model=="Generic_COOP")
    {
