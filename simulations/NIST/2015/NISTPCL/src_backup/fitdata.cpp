                         ...
                         const vector<string>& relmodel)
{
    ...
    if (write_fit_tauFit.is_open())
    {
        ...
        compute_Tg_fragility(sysVar,n_trl,n_sys,c_org,model);
    ...
    {
        cout << "fit_tauFit: 'fit_tauFit.dat' cannot open." << "\n";
        //system("read -t5");exit(EXIT_FAILURE);
    }
}
    ...
    else if (model=="FourParamVFT") // [tau0,D,T0,alpha]
    {
        if (systemUnit=="real")
        {
            if (is_use_viscosity)
            {
                ...
                set_coeffs_scale({1.0e-6, 1e+1, 1e+2, 1.0});
                set_coeffs_bndl({0.0, 0.0, 0.0, 0.0});
                                              ...
                                              const string& tcalc)
{
    ...
    if (tcalc=="KWW") {
        ...
        cout
        << "in sExp_tauFit_interpolateFvalue, " << tcalc << " model not found!" << "\n"
        << "Program aborted." << "\n";
        system("read -t5");exit(EXIT_FAILURE);
    }
                                          ...
                                          const string& tcalc)
{
    ...
    if (tcalc=="KWW") {
        ...
        cout
        << "in sExp_tauFit_gammafunction, " << tcalc << " model not found!" << "\n"
        << "Program aborted." << "\n";
        system("read -t5");exit(EXIT_FAILURE);
    }
                                ...
                                const string& tcalc)
{
    ...
    if (tcalc=="KWW") {
        ...
        cout
        << "in sExp_func_value, " << tcalc << " model not found!" << "\n"
        << "Program aborted." << "\n";
        system("read -t5");exit(EXIT_FAILURE);
    }
...
double FitData::error_Tg(const string& model)
{
    if (model=="VFT")
    {
        ...
        double time=get_extrp_time(); // time @ Tg
                                            ...
                                            const double Th)
{
    ...
    if (readfile.is_open()) {
        ...
        cout
        << "get_interpolated_Ts: readtempinfo cannot open."
        << "\n";
    }
                                       ...
                                       const double Temp_d)
{
    ...
    if (readFile.is_open()) {
        ...
        cout
        << "find_individual_msd_data: msd file cannot open."
        << "\n";
    }
                                  ...
                                  const int n_sys)
{
    ...
    if (readDWFdata.is_open()) {
        ...
        cout
        << "read_individual_DWF_data: DWF_all file cannot open."
        << "\n";
    }
                                      ...
                                      const int n_sys)
{
    ...
    if (readDWFdata.is_open()) {
        ...
        cout
        << "read_individual_equ_DWF: DWF_all file cannot open."
        << "\n";
    }
                               ...
                               const int n_sys)
{
    ...
    for (int n_trl=0; n_trl<sysVar.get_n_trial(); ++n_trl) {
        ...
        if (readDWFdata.is_open()) {
            ...
            cout
            << "read_all_equ_DWF: DWF_all file cannot open."
            << "\n";
        }
    }
                                               ...
                                               const double Temp_d)
{
    ...
    if (readFile.is_open()) {
        ...
        cout
        << "skim_individual_correlation_data: correlation file cannot open."
        << "\n";
    }
}
                                               ...
                                               const double Temp_d)
{
    ...
    if (readFile.is_open()) {
        ...
        cout
        << "read_individual_correlation_data: correlation file cannot open."
        << "\n";
    }
    ...
    try {
        ...
        cout
        << "in read_individual_correlation_data:" << "\n"
        << "correlation size = " << i << "\n";
                                         ...
                                         const int n_sys)
{
    ...
    if (readtaueqdata.is_open()) {
        ...
        cout
        << "read_individual_taueq_data: taueq datafile cannot open." << "\n";
    }
                                         ...
                                         const int n_sys)
{
    ...
    if (readtempsdata.is_open()) {
        ...
        cout
        << "read_individual_temps_data: temps datafile cannot open." << "\n";
    }
                              ...
                              const std::string& model)
{
    if (outputFile.is_open()) {
        ...
        cout
        << "write_taucutoff: file cannot open."
        << "\n";
    }
}
...
void FitData::write_errcurve(std::ofstream& outputFile,
                             const std::string& model)
{
    if (outputFile.is_open()) {
...
void FitData::write_errcurve_actual(std::ofstream& outputFile,
                                    const std::string& model,
                                    const std::vector<std::vector<double>>& vec2d_sortdecreasing)
{
    if (outputFile.is_open()) {
...
void FitData::write_errcurve_log(std::ofstream& outputFile,
                                 const std::string& model,
                                 const std::vector<std::vector<double>>& vec2d_sortdecreasing)
{
    if (outputFile.is_open()) {
                                   ...
                                   const std::string& model)
{
    ...
    //------------------------------------------------------------------
    double Tg_extrp = calc_temp_given_time(c,get_extrp_time(),model)[0];
    double m_extrp  = calc_temp_given_time(c,get_extrp_time(),model)[1];
    ...
    //------------------------------------------------------------------
    double Tg_compu = calc_temp_given_time(c,get_compu_time(),model)[0];
    double m_compu  = calc_temp_given_time(c,get_compu_time(),model)[1];
                                 ...
                                 const std::string& model)
{
    if (outputFile.is_open()) {
        ...
        outputFile
        << "-------------------------------------" << "\n"
        << "Tg_extrp [@("<<get_extrp_time()<<")timeUnit] "
        << Tg_extrp
        << "\n"
        << "m_extrp [@Tg_extrp] " << m_extrp
        << "\n\n"
        
        << "Tg_compu [@("<<get_compu_time()<<")timeUnit] "
        << Tg_compu
        << "\n"
        << "m_compu [@Tg_compu] " << m_compu
        << "\n"
        << "-------------------------------------" << "\n"
        << "\n";
        ...
        cout
        << "write_Tg_fragility: file cannot open."
        << "\n";
    }
}
                                    ...
                                    const std::string& tcalc)
{
    if (outputFile.is_open()) {
        ...
        cout
        << "write_fit_correlation: file cannot open."
        << "\n";
    }
}
...
void FitData::write_tauFit_vs_T(std::ofstream& outputFile)
{
    if (outputFile.is_open()) {
...
void FitData::write_tauFit_vs_invT(std::ofstream& outputFile)
{
    if (outputFile.is_open()) {
    ...
    if (outputFile.is_open()) {
        ...
        if (model=="Arrhenius") {
            ...
            for (int i=0; i<=equal_pieces; ++i) {
                ...
                Ti=T_initial-(abs(Tg_extrp-T_initial)/(double)equal_pieces)*i;
        ...
        cout
        << "write_tauFitfit_vs_T: file cannot open."
        << "\n";
    }
                                      ...
                                      const std::string& model)
{
    if (outputFile.is_open()) {
        ...
        cout
        << "write_tauFitfit_vs_invT: file cannot open."
        << "\n";
    }
}
                          ...
                          const int n_sys)
{
    ...
    if (write_qchTs.is_open()) {
        ...
        cout
        << "write_qchTs: 'tempsInfo_qchTs' cannot open."
        << "\n";
    }
                          ...
                          const int n_sys)
{
    ...
    if (write_equTs.is_open()&&
        ...
        // new feature (if the below block is enabled):
        ...
        cout
        << "write_equTs: 'tempsInfo_equTs' cannot open."
        << "\n";
    }
}
                      ...
                      const vector<vector<double>> tauVec)
{
    ...
    if (false) { // use log-scale tau
        ...
        if (which_T>0) {
            ...
            dT=abs(T_hi-T_lo)/(double)equal_parts;
                       ...
                       const int n_sys)
{
    ...
    if (write_TA.is_open()) {
        ...
        cout
        << "findTA: 'tempsInfo_TA' cannot open."
        << "\n";
    }
}
                         ...
                         const std::string& model)
{
    ...
    if (write_tau0.is_open()) {
        ...
        find_tau0(sysVar,n_sys,model);
        double deviation=abs(tau0_model-tau0_Arrhenius)/abs(tau0_Arrhenius);
                                  ...
                                  const std::string& model)
{
    ...
    if (n_data<n_fit_chopping) {
        cout
        << "fit_continuousRange: n_data < " << n_fit_chopping
        << " points. returned." << "\n";
        return;
    }
                                     ...
                                     const std::string& model)
{
    ...
    if (n_data<n_fit_sliding) {
        cout
        << "fit_neighboringNpoints: n_data < " << n_fit_sliding
        << " points. returned." << "\n";
        return;
    }
