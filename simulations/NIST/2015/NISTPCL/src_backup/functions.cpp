...
const string autoWork::submit_jobs()
{
    ...
    FitDielectric dielectric(species);
    
    dielectric.set_is_use_dc_free(false); // true: dc_free loss; false: raw loss
    ...
    {
        // NOTE:
        // temperatures from the dielectric experiments are in general different
        // from the simulation (i.e. done by different groups by defualt);
        // therefore, T's from dielectric are store in a separately defined
        // "tinfo_dielectric" contianer, instead of using "tinfo" container
        
        vector<vector<double>> tinfo_dielectric;
        
        /* make folder for and transfer dielectric data */
        make_dielectric_folder(species,form);
        
        /* data pre-processing */
        tinfo_dielectric=dielectric.dielectricData_preprocess(species,form);
        
        /* fit relaxation profile to the chosen model */
        for (int process=0; process<tinfo_dielectric.size(); ++process)
        {
            // NOTE:
            // if there are both cooling and heating processes:
            // process=0: cooling temperatures
            // process=1: heating temperatures
            
            for (int T=0; T<tinfo_dielectric[process].size(); ++T)
            {
                dielectric.fit_HN(species,process,tinfo_dielectric[process][T],func,form);
            }
        }
    }
                               ...
                               bool& is_retry)
{
    ...
    /* Fitting Models */
    /*--------------------------------------------------------------------------
     Toolkits for Fitting Different Relaxation Mechanisms
     ---------------------------------------------------------------------------
     ...
     ---------------------------------------------------------------------------
     Toolkits for Fitting Temperature Dependence of Relaxation Time:
     ---------------------------------------------------------------------------
     3-PARAMETER:
     ------------
...
void autoWork::make_dielectric_folder(const StructureClass& sysVar,
                                      const string& form)
{
    ...
    if (countaccess==1)
    {
        ...
        /* copy data from a specified path to the destination folder */
        ...
        /* change file names to a readable form */
