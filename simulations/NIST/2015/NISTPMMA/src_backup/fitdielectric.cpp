...
void FitDielectric::combine_domain_data_to_fit()
{
    ...
    if (n_peaks>0) {
        for (int i=0; i<n_peaks; ++i) {
            for (int ii=0; ii<(int)dielectric_freq_sortincreasing.at(i).size(); ++ii) {
                logafreq=dielectric_freq_sortincreasing.at(i)[ii][0]; // log(afreq)
                logloss =dielectric_freq_sortincreasing.at(i)[ii][1]; // log(loss)
    ...
    if ( n_dcpoints>1 || n_peaks>0 ) {
        ...
        if(logafreq_vec.size()==logloss_vec.size()) {
            ...
            dielectric_freq_domains=dielectric_freq;
        }
        else {
            cout
...
void FitDielectric::combine_global_data_to_fit()
{
    ...
    for (int i=0; i<(int)dielectric_freq_overall.size(); ++i) {
        dielectric_freq_global.push_back
        ({
...
{
    ++n_fit_local_accessed;
    
    /* sample data in the proximity of found peaks */
    if (n_fit_local_accessed==1) {
        peak_sampling_bySlope(dielectric_freq_overall);
    } else {
        //cout << "lossredu.size " << dielectric_freq_overall_reduced.size() << "\n";
    ...
    if ((!is_use_dc_free)&&(n_fit_local_accessed==1)) {
...
void FitDielectric::fit_combined_local_domains()
{
    ...
    fitdata_processing_lsfit(dielectric_freq_domains);
...
void FitDielectric::fit_global_domain()
{
    ...
    fitdata_processing_lsfit(dielectric_freq_global);
...
void FitDielectric::reduce_raw_loss()
{
    ...
    for (int i=0; i<(int)raw.size(); ++i) {
        afreq=raw[i][0];
        loss_reduced=raw[i][2]-pow(10,log10(fabs(combined_func_value(c_combined,afreq))));
        /* NOTE: here uses log value of combined_func_value is because this was
         how the loss data were used to fit; fitting always uses log values */
        if (loss_reduced>0) {
            if (((raw[i][2]-loss_reduced)/raw[i][2])<lossredu_frac) {
...
void FitDielectric::clear_containers()
{
    ...
    dielectric_freq_peak.clear();
    dielectric_freq_overall.clear();
    dielectric_freq_overall_reduced.clear();
    dielectric_freq_domains.clear();
    dielectric_freq_global.clear();
    masterloss.clear();
                                        ...
                                        double Temp_d)
{
    ...
    if( n_dcpoints>1 || n_peaks>0 )
    {
        ...
        while (r2_global_pre<r2_threshold)
        {
            ...
            if (dielectric_freq_overall_reduced.size()>3)
            {
                fit_local_domains();
                fit_combined_local_domains();
                fit_global_domain();
                r2_global_now=rep_global.r2;
                
                if(r2_global_now<=r2_global_pre) break;
                else r2_global_pre=r2_global_now;
            }
            else
            {
                break;
...
void FitDielectric::write_combined_curve(std::ofstream& outputFile)
{
    if (dielectric_freq_domains.size()>0) {
        ...
        outputFile
        << "Combined Local Domains Fit               " << "\n"
        << "c[i]                                     " << "\n"
        << "=========================================" << "\n";
        for (int i=0; i<c_combined.length(); ++i) outputFile << c_combined[i] << " ";
        outputFile
        << "\n"
        << "r2 " << rep_combined.r2 << "\n\n";
        ...
        outputFile
        << "Fit Range "
        << "(#.="<< dielectric_freq_domains.size()<<")" << "\n"
        << "log.afreq  log.loss                       " << "\n"
        << "==========================================" << "\n";
        for (size_t i=0; i<(int)dielectric_freq_domains.size(); ++i) {
            outputFile
            << dielectric_freq_domains[i][0] << " "
            << dielectric_freq_domains[i][1] << "\n";
        }   outputFile << "\n";
        
        outputFile
        << "Combined Curve of Local Domains (Full Range) " << "\n"
        << "log.afreq  log.sfreq  f_fit                  " << "\n"
        << "=============================================" << "\n";
        for (int i=0; i<=equal_pieces; ++i) {
            ...
            << afreqi << " "
            << sfreqi << " "
            << log10(fabs(combined_func_value(c_combined,pow(10,afreqi))))
            << "\n";
        } outputFile << "\n\n";
    }
}
...
void FitDielectric::write_global_curve(std::ofstream& outputFile)
{
    if (dielectric_freq_global.size()>0) {
        ...
        outputFile
        << "Attempted Global Fit                     " << "\n"
        << "c[i]                                     " << "\n"
        << "=========================================" << "\n";
        for (int i=0; i<c_global.length(); ++i) outputFile << c_global[i] << " ";
        outputFile
        << "\n"
        << "r2 " << rep_global.r2 << "\n\n";
        ...
        outputFile
        << "Fit Range "
        << "(#.="<< dielectric_freq_global.size()<<")" << "\n"
        << "log.afreq  log.loss                      " << "\n"
        << "=========================================" << "\n";
        for (size_t i=0; i<(int)dielectric_freq_global.size(); ++i) {
            outputFile
            << dielectric_freq_global[i][0] << " "
            << dielectric_freq_global[i][1] << "\n";
        }   outputFile << "\n";
        
        outputFile
        << "Combined Curve of Global Domain (Full Range) " << "\n"
        << "log.afreq  log.sfreq  f_fit                  " << "\n"
        << "=============================================" << "\n";
        for (int i=0; i<=equal_pieces; ++i) {
            ...
            << afreqi << " "
            << sfreqi << " "
            << log10(fabs(combined_func_value(c_global,pow(10,afreqi))))
            << "\n";
        } outputFile << "\n\n";
    }
}
...
void FitDielectric::write_reducedLoss(std::ofstream& outputFile)
{
    if (dielectric_freq_overall_reduced.size()>0) {
        ...
        outputFile
        << "Reduced Loss "
        << "(#.="<< dielectric_freq_overall_reduced.size()<<")" << "\n"
        << "log.afreq  log.sfreq  log.loss  storage           " << "\n"
        << "==================================================" << "\n";
        for (size_t i=0; i<(int)dielectric_freq_overall_reduced.size(); ++i) {
            outputFile
            << log10(fabs(dielectric_freq_overall_reduced[i][0])) << " "   // log(afreq)
            << log10(fabs(dielectric_freq_overall_reduced[i][1])) << " "   // log(afreq)
            << log10(fabs(dielectric_freq_overall_reduced[i][2])) << " "   // log(loss)
            << dielectric_freq_overall_reduced[i][3]              << "\n"; // storage
        } outputFile << "\n\n";
    }
}
...
void FitDielectric::write_master_curve(std::ofstream& outputFile)
{
    if (dielectric_freq_overall_reduced.size()>0) {
        ...
        for (int i=0; i<=equal_pieces; ++i) {
            ...
            alglib::spline1ddiff(interpolant,afreqi,f,df,d2f);
...
void FitDielectric::set_fitParams(const std::string& model)
{
    ...
    else if (model=="global")
    {
        vector<double>
        coeffs,coeffs_scale,coeffs_bndl,coeffs_bndu;
        for (int i=0; i<(int)c_combined.length(); ++i)
        {
            coeffs.push_back(c_combined[i]);
            coeffs_scale.push_back(c_combined[i]);
            coeffs_bndl.push_back(-inf);
            coeffs_bndu.push_back(inf);
        }
        set_coeffs(coeffs);
