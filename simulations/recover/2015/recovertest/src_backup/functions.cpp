...
const string autoWork::submit_jobs()
{
    ...
    species.set_crossTemp(460.0);    // crossing T
    species.set_hTres(10.0);         // resolution above crossTemp
    species.set_lTres(10.0);         // resolution below crossTemp
    ...
    species.set_is_defaultLmpData(0);  // Data File Generation by StructureClass
    species.set_is_moltempLmpData(1);  // Data File Generation by Moltemplate
    species.set_is_LammpsPhase(0);     // Simulation Scripts Generation
    species.set_is_Simulations(1);     // Bash Scripts Generation
    species.set_is_AMDAT(1);           // AMDAT Scripts Generation
    species.set_is_fitData(0);         // Curve-fitting toolkit with ALGLIB
    species.set_is_fit_dielectric(0);  // Fit Experimetal Dielectric Data
    ...
    /** set dynamic range covered by simulation (rarely used) **/
    //--------------------------------------------------------------------------
    ...
    /** LAMMPS datafile generated by StructureClass **/
    //--------------------------------------------------------------------------
    ////////////////////////////////////////////////////////////////////////////
    ...
    /** LAMMPS datafile generated by Moltemplate **/
    //--------------------------------------------------------------------------
    ////////////////////////////////////////////////////////////////////////////
    MoltemplateLmpData moldata(species);
    
    /** "Sequence set" passed by GA
     NOTE: sequenceNum and sequenceLen can be commented out so that their
     values won't be checked) **/
    moldata.set_sequenceNum(24); // default: 0
    moldata.set_sequenceLen(30); // default: 0
    /** TODO:replace by GA sequence set **/
    ...
    if (species.get_is_moltempLmpData())
    {
        ...
        moldata.set_moltemplateBoxSize(300.0); // default: 300.0 Angsroms
    ...
    if (species.get_is_fit_dielectric())
    {
        ...
        if (!ds.get_is_manualFitSingleDS()) {
            ...
            /* fit dielectric response to the chosen func form */
            ...
            /* fit temperature denpendence of tau to the chosen model */
                               ...
                               bool& is_retry)
{
    ...
    aa.set_is_recoverAtomType(false);    // recover modified atomtypes to original
       ...
       (species.times_retry<=species.max_times_retry)) // NOTE: "<="
    {
        ...
        if (lmp.get_is_resize()) aa.set_is_NPT(false);
        
        /** multiBody analysis **/
        aa.set_is_mbodies(false);
        ...
        /** check and make cooresponding statistic folders on the fly **/
        ...
        for (int n_trl=0; n_trl<n_trial; ++n_trl)
        {
            for (int n_sys=n_sys_beg; n_sys<=n_sys_end; ++n_sys)
            {
                ...
                for (int T=0; T<species.get_n_Temp(); ++T) {
                    ...
                    /** delete the 0th frame from 2nd to final block (this is
                     necessary for super long simulations run by using this
                     program) **/
    ...
    fd.set_relaxation_target("isfs"); // which relaxation: "isfs" or "baf"
...
const string autoWork::make_SimulationsFolders(const StructureClass& sysVar)
{
    ...
    if ( sf.is_open() )
    {
        ...
        if (sysVar.get_is_moltempLmpData()) {
            sf
            << "mkdir -p ${lammps_inputs}/start_data/moltemplate"       << "\n";
        }
                                           ...
                                           const AmdatAnalysis& aa)
{
    ...
    if(t.is_open()) {
        ...
        if (aa.get_is_baf()) {
            t
            << "baf_all="
            << path_d << "/statistics/baf/all"                          << "\n"
            << "baf_backbone="
            << path_d << "/statistics/baf/backbone"                     << "\n"
            << "test -e ${baf_all}"                                     << "\n"
            << "if [ \"$?\" -ne \"0\" ]; then"                          << "\n"
            << "  mkdir -p ${baf_all}"                                  << "\n"
            << "fi"                                                     << "\n"
...
{
    // TODO: make initialization section cleaner
}
